...
class WaypointFollowerLOSIMU(Node):
    ...
    PATH_FRAME_ID = 'map'
    PATH_POINT_SPACING_M = 0.5     # добавлять точку, если прошли >= 0.5 м
    PATH_MAX_POINTS = 2000         # ограничение на длину траектории
    ...
    def __init__(self):
        ...
        self._pub_path = self.create_publisher(Path, '/wamv/debug/path', 10)
        ...
        self._path = Path()
        self._path.header.frame_id = self.PATH_FRAME_ID
    ...
    def _update_path(self, x: float, y: float):
        if self._lat_ref is None:
            return

        if self._last_path_x is not None and self._last_path_y is not None:
            if math.hypot(x - self._last_path_x, y - self._last_path_y) < self.PATH_POINT_SPACING_M:
                return
        ...
        qz = math.sin(self._yaw * 0.5) if self._yaw is not None else 0.0
        qw = math.cos(self._yaw * 0.5) if self._yaw is not None else 1.0
        pose.pose.orientation = Quaternion(x=0.0, y=0.0, z=qz, w=qw)

        self._path.header.stamp = now
        self._path.poses.append(pose)

        if len(self._path.poses) > self.PATH_MAX_POINTS:
